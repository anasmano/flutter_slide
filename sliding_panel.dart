...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  bool _scrollingEnabled1 = false;
  bool _scrollingEnabled3 = false;
  bool _scrollingEnabled5 = false;
                          ...
                          ? Positioned(
                              ...
                              child: GestureDetector(
                                  onVerticalDragDown: (c) async {
                                    await scrollingEnabledDown();
                                  },
                                  onVerticalDragEnd: (c) async {
                                    await scrollingEnabledUp();
                                  },
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent p) {
        //_scrollingEnabled3 = false;
        //print('cccccccccccc ${_scrollingEnabled5}');
        ...
        //     //print('cZZZZZZZZ ${_scrollingEnabled1}');
        //     _scrollingEnabled3 = true;
        //     //     _onGestureSlide(p.delta.dy);
        //   } else {
        //     //print('cVVVVVVVV ${_scrollingEnabled1}');
        //     _scrollingEnabled3 = false;
        //   }
        // }
        // if (_scrollingEnabled3 == true || _scrollingEnabled5 == true) {
        //   _onGestureSlide(p.delta.dy);
        // }
        _onGestureSlide(p.delta.dy);
      },
  ...
  _onPointerDown(PointerDownEvent p) {
    _vt.addPosition(p.timeStamp, p.position);
  }

  _onPointerMove(PointerMoveEvent p) {
    _vt.addPosition(
        p.timeStamp, p.position); // add current position for velocity tracking
    _onGestureSlide(p.delta.dy);
  }

  _onPointerUp(PointerUpEvent p) {
    _onGestureEnd(_vt.getVelocity());
  }

  Future<void> scrollingEnabledDown() async {
    _scrollingEnabled5 = true;
  ...
  Future<void> scrollingEnabledUp() async {
    //print('SLIDE UPPPPPPPPPPPPPPPPPP');
    _scrollingEnabled5 = false;
    if (_scrollingEnabled1 == true) {
      _scrollingEnabled = true;
...
class PanelController {
  ...
  Future<void> scrollingEnabledDown() async {
    _panelState!._scrollingEnabled5 = true;
    if (_panelState!._scrollingEnabled == true) {
      _panelState!._scrollingEnabled = false;
      _panelState!._scrollingEnabled1 = true;
    }
  }

  Future<void> scrollingEnabledUp() async {
    //print('SLIDE UPPPPPPPPPPPPPPPPPP');
    _panelState!._scrollingEnabled5 = false;
    if (_panelState!._scrollingEnabled1 == true) {
      _panelState!._scrollingEnabled = true;
      _panelState!._scrollingEnabled1 = false;
    }
  }

  Future<void> scrollingEnabledTrue() async {
    _panelState!._scrollingEnabled = true;
  }

  Future<void> scrollingEnabledFalse() async {
    _panelState!._scrollingEnabled = false;
  }

  bool get getScrollingEnabled {
    return _panelState!._scrollingEnabled;
  }

  set setScrollingEnabled(bool en) {
    _panelState!._scrollingEnabled = en;
  }

  onGestureSlide(double dy) {
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    return _panelState!._onGestureSlide(dy);
  }

  onGestureEnd(Velocity v) {
    //assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    return _panelState!._onGestureEnd(v);
  }

  onPointerDown(PointerDownEvent p) {
    return _panelState!._onPointerDown(p);
  }

  onPointerMove(PointerMoveEvent p) {
    return _panelState!._onPointerMove(p);
  }

  onPointerUp(PointerUpEvent p) {
    return _panelState!._onPointerUp(p);
  }
}
